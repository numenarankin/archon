...
-- keep full access; a plain member needs the `view_sales` page granted in Settings.
-- Depends on app_has_capability() from 20260625001100.

alter policy sales_prospects_rw on sales_prospects
  using (workspace_id in (select app_workspace_ids()) and app_has_capability('view_sales'))
  with check (workspace_id in (select app_workspace_ids()) and app_has_capability('view_sales'));
